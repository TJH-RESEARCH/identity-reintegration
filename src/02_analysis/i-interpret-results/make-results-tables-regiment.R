...
table <- 
  texreg::texreg(
    ...
    #                      'Years of Service'
    #),
    caption = paste("Regressions: Regimentation", str_to_upper(str_remove(string = analysis, pattern ='variables/')), " Analysis"), 
    caption.above = T
  )
...
gof_stats <- texreg::texreg(
  ...
  custom.note = 'Standard error in parentheses. Bold indicates p less than .05.',
  bold = .05,
  caption = "Regressions on Regimentation", 
  caption.above = T
)
...
# Write -------------------------------------------------------------------
write_lines(x = x,
            file = paste0(here::here(), '/output/tables/regiment-tables-', analysis, '.txt'))
